      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="font-body text-sm text-cream-muted">
            {categories.length} أقسام · {totalItems} صنف
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 rounded-lg bg-ember px-4 py-2 font-body text-sm font-semibold text-cream shadow-pill transition-opacity hover:opacity-90"
          >
      ...
      <section className="mb-6 rounded-xl border border-char-line bg-char-soft p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="flex items-center gap-1.5 rounded-full bg-char py-1 pe-1.5 ps-3 font-body text-xs text-cream"
            >
              {cat.name}
          ...
          <form action={createCategory} className="flex items-center gap-1.5">
            <input
              ...
              className="w-32 rounded-full border border-char-line bg-char px-3 py-1.5 font-body text-xs text-cream outline-none focus:border-ember sm:w-40"
            />
            <button
              type="submit"
              title="إضافة قسم"
              className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-ember/20 text-ember transition-colors hover:bg-ember/30"
            >
              <Plus className="h-4 w-4" strokeWidth={2.5} />
            </button>
          </form>
      ...
      {categories.map((cat) => (
        <section key={cat.id} className="mb-8">
          <h2 className="mb-3 flex items-center gap-2 font-display text-base font-bold text-saffron">
            {cat.name}
            <span className="font-body text-xs font-normal text-cream-muted">
              ({cat.items.length})
            </span>
          </h2>
                ...
                <table className="w-full border-collapse text-right">
                  <thead>
                    <tr className="bg-char-soft">
                      <th className="px-4 py-3 font-body text-xs font-medium text-cream-muted">
                        الصورة
                      </th>
                      <th className="px-4 py-3 font-body text-xs font-medium text-cream-muted">
                        الاسم
                  ...
                  <tbody className="divide-y divide-char-line">
                    {cat.items.map((item) => (
                      <tr key={item.id} className="bg-char-soft/40">
                        ...
                        <td className="whitespace-nowrap px-4 py-3 font-body text-sm text-saffron">
                          {CURRENCY}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <Link
                              ...
                              className="flex items-center gap-1 rounded-lg border border-char-line px-2.5 py-1.5 text-xs text-cream-muted transition-colors hover:border-ember/50 hover:text-cream"
                            >
                            ...
                            <form action={deleteItem}>
                              ...
                              <ConfirmDeleteButton
                                label="حذف"
                                confirmMessage={`سيتم حذف "${item.name}" نهائياً. هل أنت متأكد؟`}
                              />
                            </form>
                          </div>
                        </td>
                  ...
                  >
                    ...
                    <div className="flex flex-shrink-0 flex-col gap-1.5">
                      <Link
                        href={`/admin/items/${item.id}/edit`}
                        title="تعديل"
                        className="rounded-lg border border-char-line p-1.5 text-cream-muted transition-colors hover:border-ember/50 hover:text-cream"
                      >
                        <Pencil className="h-3.5 w-3.5" strokeWidth={2.25} />
                      </Link>
                      <form action={deleteItem}>
...
function StatusBadge({ available }) {
  return (
    <span
      className={`inline-block rounded-full px-2 py-0.5 font-body text-[11px] ${
        available
          ? "bg-emerald-500/15 text-emerald-400"
          : "bg-char-line text-cream-muted"
      }`}
    >
